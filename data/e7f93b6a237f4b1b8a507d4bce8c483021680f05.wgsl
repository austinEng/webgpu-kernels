...
@group(0) @binding(3) var<uniform> uniforms : Uniforms;
    @compute @workgroup_size(64) fn main(@builtin(global_invocation_id) global_id  : vec3u) {
      ...
      sum += vec4<f32>(
        dot(vec4<f32>(unpack4xU8(matrix.values[4u * (global_id.x * uniforms.packedCols + col) + 0u])), v),
        dot(vec4<f32>(unpack4xU8(matrix.values[4u * (global_id.x * uniforms.packedCols + col) + 1u])), v),
        dot(vec4<f32>(unpack4xU8(matrix.values[4u * (global_id.x * uniforms.packedCols + col) + 2u])), v),
        dot(vec4<f32>(unpack4xU8(matrix.values[4u * (global_id.x * uniforms.packedCols + col) + 3u])), v),
      );
      }
